...
async function fetchPage(offset = 0) {
  ...
  if (!res.ok) throw new Error(`Piper Sandler Workday returned ${res.status}`);
  return res.json();
}
...
export async function GET() {
  try {
    ...
    console.error("Piper Sandler API error:", err);
    return Response.json({ error: "Failed to fetch Piper Sandler jobs" }, { status: 500 });
  }
}
