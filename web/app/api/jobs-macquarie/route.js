...
async function fetchPage(offset = 0) {
  ...
  if (!res.ok) throw new Error(`Macquarie Workday returned ${res.status}`);
  return res.json();
}
...
export async function GET() {
  try {
    ...
    console.error("Macquarie API error:", err);
    return Response.json({ error: "Failed to fetch Macquarie jobs" }, { status: 500 });
  }
}
