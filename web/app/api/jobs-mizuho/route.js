...
async function fetchPage(offset = 0) {
  ...
  if (!res.ok) throw new Error(`Mizuho Workday returned ${res.status}`);
  return res.json();
}
...
export async function GET() {
  try {
    ...
    console.error("Mizuho API error:", err);
    return Response.json({ error: "Failed to fetch Mizuho jobs" }, { status: 500 });
  }
}
