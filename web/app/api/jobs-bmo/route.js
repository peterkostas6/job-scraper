...
async function fetchPage(offset = 0) {
  ...
  if (!res.ok) throw new Error(`BMO Workday returned ${res.status}`);
  return res.json();
}
...
export async function GET() {
  try {
    ...
    console.error("BMO API error:", err);
    return Response.json({ error: "Failed to fetch BMO jobs" }, { status: 500 });
  }
}
