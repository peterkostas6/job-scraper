...
async function fetchPage(offset = 0) {
  ...
  if (!res.ok) throw new Error(`MUFG Workday returned ${res.status}`);
  return res.json();
}
...
export async function GET() {
  try {
    ...
    console.error("MUFG API error:", err);
    return Response.json({ error: "Failed to fetch MUFG jobs" }, { status: 500 });
  }
}
