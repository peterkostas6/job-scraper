...
async function fetchPage(offset = 0) {
  ...
  if (!res.ok) throw new Error(`Houlihan Lokey Workday returned ${res.status}`);
  return res.json();
}
...
export async function GET() {
  try {
    ...
    console.error("Houlihan Lokey API error:", err);
    return Response.json({ error: "Failed to fetch Houlihan Lokey jobs" }, { status: 500 });
  }
}
